...
def _object(view: jsonschema_rs.canonical.ObjectView, ctx: StrategyContext) -> SearchStrategy[JsonValue]:
    ...
    if names is not None:
        ...
        # Canonicalization keeps only the names an actual key could spell, so this is a string strategy.
        source = _text(ctx) if view.property_names is None else from_schema(view.property_names, ctx)
        free_names = cast("SearchStrategy[str]", source).filter(lambda key: key not in known)
...
def _sized_object(
    ...
    if free_names is None:
        return result
    # A floor the named keys cannot reach outranks the ceiling on how many extras are drawn by default.
    extra_low = max(0, minimum - len(result))
    budget = _EXTRA_KEYS if maximum is None else min(_EXTRA_KEYS, maximum - len(result))
    extra = draw(st.dictionaries(free_names, unnamed, min_size=extra_low, max_size=max(extra_low, budget)))
    return {**extra, **result}
...
def _string(view: jsonschema_rs.canonical.StringView, ctx: StrategyContext) -> SearchStrategy[JsonValue]:
    ...
    # only would be sound but narrow: `^x` would spell one single string, and a `minLength` beside it
    # nothing at all.
